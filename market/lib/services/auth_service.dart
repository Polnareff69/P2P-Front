...
class AuthService {
  ...
  bool get isLoggedIn => _currentToken != null && !isTokenExpired();
  bool get isSeller => _currentRole?.toLowerCase() == AppConstants.sellerRole;
  bool get isUser => _currentRole?.toLowerCase() == AppConstants.userRole;
  ...
  Future<void> initialize() async {
    try {
      ...
      if (kDebugMode) print('❌ Error inicializando AuthService: $e');
  ...
  Future<void> saveToken(String token) async {
    try {
      ...
      if (kDebugMode) print('✅ Token guardado exitosamente');
    } catch (e) {
  ...
  Future<void> _decodeAndSaveUserInfo(String token) async {
    try {
      ...
      await prefs.setString(AppConstants.userRoleKey, _currentRole!);
      await prefs.setString(AppConstants.userEmailKey, _currentEmail!);
      await prefs.setString(AppConstants.usernameKey, _currentUsername!);
      ...
      if (kDebugMode) print('❌ Error decodificando token: $e');
  ...
  Future<void> _loadUserData() async {
    try {
      if (kDebugMode) print('📱 Cargando datos guardados...');
      
      final prefs = await SharedPreferences.getInstance();
      
      _currentToken = prefs.getString(AppConstants.tokenKey);
      _currentRole = prefs.getString(AppConstants.userRoleKey);
      _currentEmail = prefs.getString(AppConstants.userEmailKey);
      _currentUsername = prefs.getString(AppConstants.usernameKey);
      
      // Verificar si el token existe pero está expirado
      if (_currentToken != null && isTokenExpired()) {
        if (kDebugMode) print('⚠️ Token expirado, limpiando datos');
        await logout();
      } else if (_currentToken != null) {
        if (kDebugMode) print('✅ Usuario logueado encontrado');
      } else {
  ...
  bool isTokenExpired() {
    ...
    try {
      ...
      if (kDebugMode) print('❌ Error verificando expiración: $e');
      return true;
    }
  }
  ...
  Map<String, dynamic> getUserInfo() {
    return {
      ...
      'tokenExpired': _currentToken != null ? isTokenExpired() : null,
  ...
  Future<bool> refreshToken() async {
    ...
    if (kDebugMode) print('🔄 Refresh token - No implementado aún');
    return false;
  }
  
  // 🛡️ VALIDAR SI EL USUARIO TIENE PERMISOS
  bool hasRole(String role) {
    return _currentRole?.toLowerCase() == role.toLowerCase();
  }
  ...
  void printCurrentState() {
    if (kDebugMode) {
      ...
      print('🔐 Token: ${_currentToken != null ? "✅ Existe" : "❌ No existe"}');
      print('👤 Usuario: $_currentUsername');
      print('📧 Email: $_currentEmail');
      print('🎭 Rol: $_currentRole');
      print('✅ Logueado: $isLoggedIn');
      print('⏰ Token expirado: ${_currentToken != null ? isTokenExpired() : "N/A"}');
      print('=========================================\n');
    }
  }
}